...
class MultitouchTest {
    ...
    static int lines = 1, rings = 0, colorChanging = 0, numberShowing = 1,
            coordinates = 1, density = 1, vibration = 1;
    static String densityText="density", centerMessage="center", currentTouches="touch";
}


/**
 * This is a draft/debug implementation.
 * It notably draws touch input.
 * We tried to use it to filter touch events one displays with broken touch panel doing ghost input.
 * However we could not find a way to filter touch input like we can filter keyboard input.
 */
public class FxOverlay extends FrameLayout {
    ...
    public FxOverlay(Context context) {
        super(context);
        ...
        if (getResources().getConfiguration().orientation == Configuration.ORIENTATION_PORTRAIT) {
            MultitouchTest.screenHeight = metrics.heightPixels;
            MultitouchTest.screenWidth = metrics.widthPixels;
        } else {
            MultitouchTest.screenHeight = metrics.heightPixels;
            MultitouchTest.screenWidth = metrics.widthPixels;
        }
        ...
        numberPaint.setTextSize(MultitouchTest.screenDensity / 10f);

        messagePaint.setStrokeWidth(1);
        messagePaint.setARGB(255, 255, 255, 255);
        messagePaint.setStyle(Paint.Style.FILL);
        messagePaint.setTextSize(MultitouchTest.screenDensity / 10f);
    ...
    public FxOverlay(Context context, AttributeSet attrs) {
        super(context, attrs);

    }

    public FxOverlay(Context context, AttributeSet attrs, int defStyle) {
    ...
    protected void onDraw(Canvas canvas) {
        ...
        String currentTouches = MultitouchTest.currentTouches
                + ": " + counting;
